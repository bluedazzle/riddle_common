...
class AnswerView(CheckTokenMixin, StatusWrapMixin, JsonResponseMixin, DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if obj.right_answer_id != aid:
            ...
            self.user.save()
            return self.render_to_response({'answer': False, 'tag': tag, 'cash': 0, 'reward': False, 'video': video})
        ...
        self.user.current_level += 1
        self.user.save()
        return self.render_to_response({'answer': True, 'tag': tag, 'cash': cash, 'reward': reward, 'video': video})
